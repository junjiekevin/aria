...
import type { CalendarEventRepository, CalendarOperationLogRepository } from '../../domain/calendar';

export async function syncSelectedCalendarsForUser(deps: {
  accounts: CalendarAccountRepository;
  providerCalendars: ProviderCalendarRepository;
  cursors: CalendarSyncCursorRepository;
  events: CalendarEventRepository;
  operations: CalendarOperationLogRepository;
  adapter: CalendarProviderAdapter;
}, input: { userId: string; sourceSurface: 'system_sync' | 'calendar_ui' }): Promise<{ syncedCalendars: number; syncedEvents: number }> {
  const account = await deps.accounts.getByUserAndProvider(input.userId, 'google');
  if (!account) return { syncedCalendars: 0, syncedEvents: 0 };
...
export async function registerGoogleWatchForSelectedCalendars(deps: {
  accounts: CalendarAccountRepository;
  providerCalendars: ProviderCalendarRepository;
  cursors: CalendarSyncCursorRepository;
  adapter: CalendarProviderAdapter;
}, input: { userId: string }): Promise<{ watchedCalendars: number }> {
  const account = await deps.accounts.getByUserAndProvider(input.userId, 'google');
  if (!account) return { watchedCalendars: 0 };
