...
const TOOL_SIGNATURES: Record<string, string> = {
    ...
    proposeScheduleChanges: 'changes[], schedule_id?',
...
7. **CALENDARS vs EVENTS**: Calendars are containers ("Primary"). Events are slots inside them ("Piano", "Singing"). If the user targets an event by name and no schedule_id is in context, call listSchedules then searchEventsInSchedule.
...
function compactToolSelection(message: string, context: PromptContext): string[] {
    ...
    if (asksBulkSchedule) {
        return context.scheduleId
            ...
            : ['listSchedules', 'listUnassignedParticipants', 'proposeScheduleChanges'];
    }
...
export function detectToolsFromIntent(
    ...
    if (suggestsSpotsForUnscheduled) {
        return context.scheduleId
            ...
            : ['listSchedules', 'listUnassignedParticipants', 'getEventSummaryInSchedule', 'proposeScheduleChanges'];
    }
